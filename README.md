...
| 部署后验证 | 重启 root PM2 `dayu-backend` 后运行 `python3 scripts/check_mcp_drift.py`，必须无 drift / warning |
...
│   └── sales-consultation.md # 销售顾问话术（异议/选型/留资预筛）
├── scripts/
│   └── check_mcp_drift.py   # skill.json vs server 漂移 + schema/价格/smoke 校验
├── assets/
